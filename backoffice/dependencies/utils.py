...
def remove_host_from_header(request):
    nextHeader = {}
    allowedHeader = [
        "x-target",
        "x-ext-authz",
        ...
        "x-level",
        "content-type",
        "x-eauthz-check-result",
        "x-uid",
    ]
    for n in request.headers:
        if n in allowedHeader:
            nextHeader[n] = request.headers[n]
    return nextHeader
...
class BaseCRUD:
    ...
    def log(self, msg):
        logger.info(self.classModel.__tablename__+"-"+str(msg))
        ...
        **whereClause
        ...
        try:
            ...
            if not data:
                ...
                await connection.update_data(session, self.classModel, data, pydanticModel)
                res = create_response(status=status.success())
        except Exception as e:
            logger.error(str(e))
            res = create_response(status=status.error(e))
